...
void addRoundKey(int currRound, char * key_schedule, char * state)
{
    for (int i = 0; i < 16; i++)
    {
        state[i] = state[i] ^ key_schedule[currRound*16 + i]; // update current state
    }
}
...
void subBytes(char * state)
{
    for (int i = 0; i < 16; i++)
    {
        state[i] = subWord(state[i]); // update current state
    }
}
...
int main ()
{
    ...
    if (run_mode == 0) {
        ...
        while (1) {
            // clear registers that are not displayed on hex drivers
            AES_PTR[4]  = 0;
            AES_PTR[5]  = 0;
            AES_PTR[6]  = 0;
            AES_PTR[7]  = 0;
